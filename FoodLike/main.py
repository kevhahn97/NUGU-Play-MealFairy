...
class Response:
    def __init__(self, req):
        ...
        self.res['output'] = {}
        self.set_parameters(
            {'FL_food': req['action']['parameters']['FL_food']['value']})
    ...
    def make_response(self, req, ID):
        if ID == 'null':
            self.set_parameters({
                'FL_response': '음식 좋아요 기능은 계정을 연동하셔야 사용하실 수 있어요. 누구 앱 좌측 메뉴에서 집밥 요정 Play를 찾아 계정을 연동해 주세요. 집밥 요정에 익숙하지 않으시다면 도움말 들려줘. 라고 말씀해 보세요'
            })
        elif ID == 'expired':
            self.set_parameters({
                'FL_response': '죄송합니다. 계정 정보를 가져오는 과정에서 오류가 발생했어요. 잠시 후에 다시 시도해 주세요.'
            })
        else:
            try:
                p_food = req['action']['parameters']['FL_food']['value']
                ...
                if rows == None: #food not ready
                    self.set_parameters({
                        'FL_response': '죄송합니다. 아직 배우고 있는 음식이에요.'
                    })
                else:
                    food_name = rows['food']
                    ...
                    rows = cur.fetchone()
                    if rows != None:
                        res = food_name + ', 이미 좋다고 하셨어요.'
                        self.set_parameters({
                            'FL_response': res
                        })
                    else:
                        sql = """insert into food_logs values (%s, default, 2, %s)"""
                        cur.execute(sql, (ID, food_name))
                        conn.commit()
                        res = food_name + ', 좋아하신다는 것 참고할게요.'
                        self.set_parameters({
                            'FL_response': res
                        })
            except:
